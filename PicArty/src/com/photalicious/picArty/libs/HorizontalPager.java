 ...
 */
package com.photalicious.picArty.libs;
...
public final class HorizontalPager extends ViewGroup {
	...
	protected void onMeasure(final int widthMeasureSpec,
		...
		if (mFirstLayout) {
			...
			final int newY = mNextScreen * displayHeight;
		...
		if (isHorizontal)
			...
			for (int i = 0; i < count; i++) {
				...
				if (child.getVisibility() != View.GONE) {
					if (i == 0) {
						child.getHeight();
						child.layout(0, measuredHeight, r - l, measuredHeight
								+ (int) (pageHeight));
			...
			// final View child = getChildAt(i);
			// if (child.getVisibility() != View.GONE) {
			// final int childHeight = child.getMeasuredWidth();
	...
	public boolean onTouchEvent(final MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (xMoved && isHorizontal) {
				// Scroll if the user moved far enough along the X axis
				mTouchState = TOUCH_STATE_HORIZONTAL_SCROLLING;
			} else if (yMoved && !isHorizontal) {
				// Scroll if the user moved far enough along the X axis
				mTouchState = TOUCH_STATE_VERTICAL_SCROLLING;
			}
		...
		case MotionEvent.ACTION_UP:
			if (isHorizontal) {
				...
				mOnScreenSwitchListener.onTouch(isTouch);
				movedX = 0;
			} else {
				if (movedY == 0) {
					isTouch = true;
			...
			if ((mTouchState == TOUCH_STATE_HORIZONTAL_SCROLLING)
				...
				if (velocityX > mDensityAdjustedSnapVelocity) {
					if (mCurrentScreen == 0) {
						...
						mOnScreenSwitchListener
								.onChangeStartEnd(getChildCount() - 1);
					} else
					...
					if (mCurrentScreen == getChildCount() - 1) {
						...
						mOnScreenSwitchListener.onChangeStartEnd(0);
					} else
				...
				if (velocityY > mDensityAdjustedSnapVelocity) {
					if (mCurrentScreen == 0) {
						setCurrentScreen(getChildCount() - 1, false);
						mOnScreenSwitchListener
								.onChangeStartEnd(getChildCount() - 1);
			...
			if (isHorizontal) {
				mScroller
						.startScroll(
								...
								deltaX,
								0,
								(int) (Math.abs(deltaX) / (float) getWidth() * ANIMATION_SCREEN_SET_DURATION_MILLIS));
			} else {
				mScroller
						.startScroll(
								0,
								...
								0,
								deltaY,
								(int) (Math.abs(deltaY) / (float) getHeight() * ANIMATION_SCREEN_SET_DURATION_MILLIS));
			}
		} else {
			if (isHorizontal) {
	...
	public static interface OnScreenSwitchListener {
		...
		void onChangeStartEnd(int screen);
	}
}
